...
// ── CLIENT-SIDE CACHE (loaded from API) ──────────────────────────
// DB is now just a cache — all real data lives in backend/data/db.json
let DB = {
  users: [],
  applications: [],
  licenses: [],
...
// Notifications are stored in backend — no local push needed
function pushNotification() {} // kept for compatibility, no-op
...
function renderNotifList() {
  ...
  const mine = (DB.notifications || []).filter(n => n.userId === currentUser.id).slice().reverse();
  if (!mine.length) {
  ...
  list.innerHTML = mine.map(n => {
    ...
    const timeStr = formatNotifTime(new Date(n.time));
...
function logout() {
  API.logout().catch(()=>{});
  currentUser = null;
  DB = { users:[], applications:[], licenses:[], notifications:[] };
...
function buildAdminShell() {
  ...
  // Load data from API then build sidebar
  Promise.all([API.getApplications(), API.getLicenses()])
    .then(([appsData, licsData]) => {
      DB.applications = appsData.applications;
      DB.licenses     = licsData.licenses;
      ...
      document.getElementById('appSidebar').innerHTML = `
        ...
        <div class="sb-item" onclick="showPage('applications',null,this)">All Applications <span class="sb-badge" id="pendCount">${pending}</span></div>
...
function renderAdminOverview() {
  ...
  const actHTML = recent.map(a=>{
    const u = a.applicant || DB.users.find(x=>x.id===a.applicant_id) || {};
...
function openReview(idOrLicId, mode) {
  ...
  if (mode==='approve'||mode==='reject') {
    ...
    if (appObj) { uObj=appObj.applicant || DB.users.find(x=>x.id===appObj.applicant_id) || {}; }
  } else {
...
function approveApp(id, note) {
  API.approveApplication(id, note)
    .then(data => {
      toast('Approved! License issued: ' + data.license_id, 'ok');
      // Push local notification for current session
      pushNotification(currentUser.id === DB.applications.find(a=>a.id===id)?.applicant_id
        ? currentUser.id : null, 'approved', 'License Approved!',
        `License ID: ${data.license_id}`);
      // Refresh data from API
      return Promise.all([API.getApplications(), API.getLicenses()]);
    })
    .then(([appsData, licsData]) => {
      DB.applications = appsData.applications;
      DB.licenses     = licsData.licenses;
...
function buildApplicantShell() {
  ...
  Promise.all([API.getApplications(), API.getLicenses(), API.getNotifications()])
    .then(([appsData, licsData, notifData]) => {
      DB.applications  = appsData.applications;
      DB.licenses      = licsData.licenses;
      DB.notifications = notifData.notifications || [];
      renderNotifBell();
      renderApplicantDash();
    })
    .catch(err => toast('Failed to load data: ' + err.message, 'err'));
...
function doVerify() {
  const licId = document.getElementById('pubVerifyInput').value.trim();
  if (!licId) { toast('Please enter a license number.','err'); return; }
  const btn=document.getElementById('verifyBtn');
  btn.innerHTML='<span class="spin"></span>'; btn.disabled=true;
  API.verifyLicense(licId)
    .then(data => {
      btn.textContent='Verify →'; btn.disabled=false;
      const found = data.license;
      const u = data.doctor || {};
      const ra = document.getElementById('verifyResult');
      ...
      ra.innerHTML = `
        <div class="res-banner ${ok?'rb-ok':'rb-fail'}">
          ...
          <div class="rb-text"><h3>License ${ok?'Verified — Valid & Active':'Status: '+cap(found.status)}</h3>
          <p>${ok?'This medical license is authentic and currently active in the national registry.':'This license is not currently active.'}</p></div>
        ...
        <div class="doc-result-card">
          ...
          <div class="dr-info">
            ...
            <p style="font-size:11px;color:var(--grey)">License issued by ${found.issued_by}</p>
          </div>
        </div>
        <div class="info-grid-2">
          ...
          ${u.email?`<div class="ic"><div class="ic-l">Email</div><div class="ic-v" style="font-size:11px">${u.email}</div></div>`:''}
        </div>
        <div class="qr-result">
          <div class="qr-result-info"><h4>QR Verification Code</h4><p>Scan to re-verify this doctor's license.</p></div>
          <div class="qr-box" id="qrBox"></div>
        </div>`;
      ra.scrollIntoView({behavior:'smooth'});
      setTimeout(()=>{
        try { new QRCode(document.getElementById('qrBox'),{text:'MedLicense:'+found.license_id+'|'+found.status+'|'+(u.name||''),width:92,height:92,colorDark:'#0A1628',colorLight:'#ffffff'}); }
    ...
    .catch(err => {
      ...
      const ra = document.getElementById('verifyResult');
      ra.classList.add('show');
      ra.innerHTML = `<div class="res-banner rb-fail"><div class="rb-icon rbi-fail">&#x274C;</div><div class="rb-text"><h3>License Not Found</h3><p>This license number does not exist in the registry.</p></div><div class="rb-id" style="color:var(--danger)">${licId}</div></div>`;
      ra.scrollIntoView({behavior:'smooth'});
    });
}
...
function doVerifyByName() {
  ...
  API.verifyByName(name)
    .then(data => {
      document.getElementById('pubVerifyInput').value = data.license.license_id;
      switchPubTab('num', document.querySelector('.s-tab'));
      doVerify();
    })
    .catch(() => toast('No doctor or license found with that name.','err'));
}
...
function suNext(step) {
  if(step===1){
    ...
    // username uniqueness checked server-side on submit
...
function checkUsernameAvail(val){
  ...
  msg.innerHTML='<span style="color:var(--grey)">Checking…</span>';
  fetch('/api/auth/check-username?username='+encodeURIComponent(val))
    .then(r=>r.json())
    .then(data=>{
      msg.innerHTML = data.taken
...
function submitSignup(){
  ...
  var userData = {
    ...
    state:          document.getElementById('su_state') ? document.getElementById('su_state').value : '',
    hospital:       document.getElementById('su_hospital').value.trim(),
    experience:     document.getElementById('su_exp').value,
    reg_no:         document.getElementById('su_reg') ? document.getElementById('su_reg').value.trim() : '',
    avatar:         pickedAvatar
  };
  ...
  API.signup(userData)
    .then(data => {
      ...
      document.getElementById('su_created_preview').innerHTML=
        ...
        '<strong style="color:#1A2340;font-size:14px">'+data.user.name+'</strong><br>'+
        'Username: <strong style="color:var(--teal)">@'+data.user.username+'</strong><br>'+
        'Specialization: '+data.user.specialization+' · Joined: '+data.user.joined;
      goToStep(5);
    })
    .catch(err => {
      btn.textContent='Create My Account'; btn.disabled=false;
      showErr('su_err4', err.message);
    });
}
...
function viewDocs(appId) {
  ...
  const u = app.applicant || DB.users.find(x=>x.id===app.applicant_id) || {};
